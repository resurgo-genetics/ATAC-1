...
+ therefore using command line to commit
+ for file: `git add Master_peaks/R/ATAC_peaks.Rmd \ git commit -m "changed file" \ git push`
+ for dir: `git add Master_peaks/R \ git commit -m "updated dir" \ git push`
<br>    
...
chr <- c(chr,"chrX","chrY")
mus <- ChIPQCsample("Mus_Treg_rep1.sorted.uniq.bam",peaks="Mus_Treg_rep1.peaks.atac.bed.txt",annotation = "mm9", chromosomes=chr)

# look at sample object
mus
...
peaks_df <- as.data.frame(peaks_mus)

# write out
write.csv(df, "mus_rep1_chipqc_peaks.csv", row.names=F)

# checking on genome browser
...
# strsplit(peaks[1], split=".peaks.atac.bed.txt")

# function to generate chipqc object and peak table with read counts in rm
# exports ChIPQCsample object and peak table
# ARGS: 

peak_rpm <- function(peaks,reads) {
...
# compute the number of peaks >=n rpm in each sample
...
# write to file
write.csv(ppf, "peaks_totals_rpm_filtered.csv")
