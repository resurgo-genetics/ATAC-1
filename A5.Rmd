...
** average replicate values and generate read count matrix**
...
+ run same matrix through APcluster
**clustering by hand**
...
#### Draw scatterplot matrix for all pair-wise comparisons
```{r scatmat}
## --- import peak file  ----------------------------------------------------------------------------------------
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A5-all-ttreg/2rpm-filtered/RDa/"
n <- 2
...
ttrqn.log2 <- data.frame(lapply(ttrqn.df[4:19], log, 2))
...
# set path to save 
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A5-all-ttreg/2rpm-filtered/plots/"
n <-2 # rpm cutoff for filename

# plot all ttreg
jd_ggscatmat(ttrqn.log2[c(1:6,15:16)])
# save
ggsave(paste(path,'a5-',n,'rpm-filtered-ttreg-pairs-rpkm.png',sep=""), width=11, height=11)

# colon vs matching spleen
jd_ggscatmat(ttrqn.log2[c(1:4,7:10)])
...
+ so far this doesn't improve clustering over no scale
...
#### Expression of genes near differential peak clusters 
+ mapped regions in each cluster to all nearby genes (GREAT)
+ imported t-treg microarray GE 
+ created matrix of GE values for genes in GO categories enriched in each cluster
+ heatmap genes
```{r cluster_go_GE}
## --- import files -----------------------------------------------------------------------------------------------
# treg ge df
path <- '/Volumes/JD_WD2/JRD_CBDMLab/JRD_CBDM_public_seqanalyses/Microarray/GCT_given/working/'
save(expcvf, file = paste0(path,'ttreg_mus_spl_vat_cln_expcvf', '.RDa'))

# path
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A5-all-ttreg/2rpm-filtered/apclusters-great/"

# cluster5+7 - genes in ATPase ontology
file <- 'c5plus7_nucleoside_triphos_genes.txt'
atp_genes <- read.delim(file = paste0(path,file), sep='\t')
atp_genes <- atp_genes[1] # remove extraneous cols
names(atp_genes) <- 'GeneSymbol' # rename column


# cluster 5 - vat genes 
file <- 'c5_vat_combo_genes.txt'
vat_genes <- read.delim(file = paste0(path,file), sep='\t')
vat_genes <- vat_genes[1]
vat_genes <- unique(vat_genes)

## --- merge genes with expression ---------------------------------------------------------------------------------
library(dplyr)

## atp genes 
atp_ge <- inner_join(atp_genes, expcvf[1:7], by = 'GeneSymbol')
# re-arrange column order
atp_ge <- atp_ge[c(1,7,6,2,4,3,5)]
# convert to matrix
rownames(atp_ge) <- atp_ge$GeneSymbol
atp_ge.mat <- as.matrix(atp_ge[-1])
atp_ge.mat <- apply(atp_ge.mat, 2, log,2)

## vat genes
vat_ge <- inner_join(vat_genes, expcvf[1:7], by = 'GeneSymbol')
# re-arrange column order
vat_ge <- vat_ge[c(1,7,6,2,4,3,5)]
# convert to matrix
rownames(vat_ge) <- vat_ge$GeneSymbol
vat_ge.mat <- as.matrix(vat_ge[-1])
vat_ge.mat <- apply(vat_ge.mat, 2, log,2)
...
# plot every other gene
pheatmap(atp_ge.mat[seq(from =1, to = nrow(atp_ge.mat), by = 2),], color = niceCols, cluster_cols = F)
pheatmap(atp_ge.mat, color = niceCols, cluster_cols = F)


pheatmap(vat_ge.mat, color = niceCols, cluster_cols = F)


## --- save ------------------------------------------------------------------------------------------------------
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A5-all-ttreg/2rpm-filtered/apclusters-great/"

pdf(paste0(path,"atp-ge.pdf"), width=6, height=6)
pheatmap(atp_ge.mat[seq(from =1, to = nrow(atp_ge.mat), by = 2),], color = niceCols, cluster_cols = F)
dev.off()

pdf(paste0(path,"vat-ge.pdf"), width=6, height=6)
pheatmap(vat_ge.mat, color = niceCols, cluster_cols = F)
dev.off()

png(paste0(path,"vat-ge.png"), width=600, height=600)
pheatmap(vat_ge.mat, color = niceCols, cluster_cols = F)
dev.off()
...
#### Enriched TFBS in differential peak clusters
...
findMotifsGenome.pl a5-ttreg-2rpm-fil-qn-fc2-pass-apclust5.bed.txt mm9 a5-apk9-c5-motifs-allbg-200/ -size 200 -mask -preparsedDir new/
```
<br> 

#### Annotation of differential peaks in ChIPseeker
```{r annot_varpeaks}
##---- load libraries and peak clusters --------------------------------------------------------------------------

# install mm9 transcript db
...
# load files for specific clusters
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A5-all-ttreg/2rpm-filtered/bed/"
files <- list.files(path = path, pattern='pass-apclust[5-7].bed.txt', full.names=T )

# get promoters 
txdb <- TxDb.Mmusculus.UCSC.mm9.knownGene
proms <- getPromoters(TxDb = txdb, upstream = 1000, downstream = 1000)

# make a list of peak (aka "tag" in this package) list
pfl <- lapply(files, readPeakFile) #[p]eak[f]ile[l]ist

# name list elements
names <- list.files(path = path, pattern='pass-apclust[5-7].bed.txt')
names1 <- sub(pattern = '^a5-ttreg-2rpm-fil-qn-fc2-pass-*(\\w+)*.bed.txt', replacement = '\\1',names)
names(pfl) <- names1

# make tag matrix list around promos
tml.proms <- lapply(pfl, getTagMatrix, windows = proms) #[t]ag[m]atrix[l]ist[prom]oter[s]

# plot peak profiles around TSS
plotAvgProf(tml.proms, xlim=c(-1000,1000), facet='row')

# annotate peaks with genomic features
library('org.Mm.eg.db')
peakAnnot <- lapply(pfl, annotatePeak,tssRegion=c(-1000,1000),
                         TxDb=txdb, annoDb ="org.Mm.eg.db")

# draw genomic feature plots
plotAnnoBar(peakAnnot)
plotDistToTSS(peakAnnot)

# functional enrichments
library(clusterProfiler)
genes <- lapply(peakAnnot, function(i) as.data.frame(i)$geneId) # define genes
#names(genes) <- sub("_","\n",names(genes))

# need to fix so organism is mouse
comp.MF <- compareCluster(geneClusters = genes[[1]], 
                          fun = 'enrichGO',
                          ont = 'MF',
                          pvalueCutoff = 0.05,
                          pAdjustMethod = "BH")
