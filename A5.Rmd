...
+ Pseudo:
++ Plot distributions of merged peak counts for all samples
...
par(oma = c(4,1,0.5,1))
boxplot(log(ttregavg,2), las = 2, cex.axis = 0.7, 
        ylab='rep average log2rpkm', main='merged atac peaks, n=37756, replicate averages')
dev.off()

# export df to file
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A5-all-ttreg/"
write.csv(ttregavg, file=paste(path,'a5-ttreg-rep-avg-counts-rpkm.csv',sep=""), row.names=F)

# convert values to log2 and convert df to matrix
ttregl2 <- sapply(ttregavg, log,2) #ttreg[l]og2

# save
save(ttregl2, file=paste(path,'a5-ttreg-rep-avg-counts-rpkm-mat.RDa',sep=""))
```
<br>  
...
+ TODO
+ do this with chr,start,end above
+ then set gene names or functional element as row.names in matrix?
<br>
...
+ Pseudo:
++ Take random samples of different sizes and compare read count distribution to population
++ Compare distributions of sample peak heights to population
...
for(i in 1:length(sizes)) {
  m <- matrix(nrow=1000,ncol=ncol(ttregl2),data = 0)
  m <- ttregl2[sample(nrow(ttregl2), size=sizes[i],replace=F),]
  sl[[i]] <- m
}
...
mpl <- melt(ttregl2)[-1] # [m]elted[p]eak[l]ist
...
#### Use samples for clustering start
+ Pseudo:
...
```{r clust}
# import peak rpkm matrix
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A5-all-ttreg/"
filename <- 'a5-ttreg-rep-avg-counts-rpkm-mat.RDa'
...
# draw parallel coordinates plot aka matplot to see expression levels
# have to transpose mat for matplot
matplot(t(dat[set1,]),
        type='l',lwd=2,col='skyblue',lty=1,
        ylim=c(8,14), xlab='Time',ylab = 'Log2 expression value')
...
pheatmap(sl[[3]], color= niceCols, clustering_distance_rows = 'canberra', clusterin_distance_cols = 'canberra')

pheatmap(sl[[3]], color= niceCols, clustering_distance_rows = 'maximum', clusterin_distance_cols = 'maximum')

pheatmap(sl[[3]], color= niceCols, clustering_distance_rows = 'minkowski', clusterin_distance_cols = 'minkowski')

pheatmap(sl[[3]], color= niceCols, clustering_distance_rows = 'correlation', clusterin_distance_cols = 'correlation')
