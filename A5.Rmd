...
## try kmeans argument to pheatmap
library(dplyr)
set.seed(123456)
...
sl3df <- data.frame(peak = rownames(sl[[3]]), sl[[3]], row.names=NULL)
...
# order df
sl3dfco <- sl3dfc[order(sl3dfc$cluster),] # ordered
...
pheatmap(as.matrix(sl3dfco2[2:9]), cluster_rows = F, color = niceCols)

# try matplotting some clusters
...
# convert back to matrix with rownames == peakID? reason for this?

k11 <- k$cluster[k$cluster == '11']
