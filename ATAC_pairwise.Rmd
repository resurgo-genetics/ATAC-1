...
g + geom_boxplot() +
  ...
  xlab('rpm cutoff') + 
  theme_bw()
ggsave(paste0(path, "a6-peakcut-boxplot.pdf"), width=7, height=5) # export pdf
```
<br>     
...
ggsave(paste0(path, "a6-boxplot.pdf"), width=7, height=5) # export pdf
...
ggsave(paste0(path,"a6-boxplot-",n,"rpm-filtered.pdf"), width =7, height =5) # export pdf
...
g + geom_histogram(fill="white",color="black",binwidth=0.1) +
  ...
  theme_bw()
ggsave(paste0(path, "a6-count-hist.pdf"), width=5, height=25)
```
<br>    
...
pdf(paste(path,Tissue[1],'-vs-',Tissue[2],'reads.heat.pdf',sep=""), width=10, height=10, pagecentre = T)
par(oma = c(6,6,6,6))
