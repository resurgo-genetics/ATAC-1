...
library(dplyr)
Cln_Nrp1n <- row_stats2(ttreg, "6","7")
Cln_Nrp1p <- row_stats2(ttreg, "8","9")
Mus <- row_stats2(ttreg, "10","11")
Spl_wCln_Nrp1n <- row_stats2(ttreg, "12","13")
Spl_wCln_Nrp1p <- row_stats2(ttreg, "14","15")
Spl_wMus <- row_stats2(ttreg,"16","17")
Spl_wVat <- row_stats2(ttreg,"18","19")
Vat <- row_stats2(ttreg, "20","21")
...
# filter by cv <= 0.6
library(dplyr)
n <- 0.6 # set cv filter value
cv06.list <- lapply(ttreg.stats, filter, CV <= n) # filter

names(cv06.list) <- psl.names  # name list elements (recycle previous names)
lapply(cv06.list, head) # check
...
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A7-all-ttreg/1rpm-cv06/plots/"
n <- 1 # set rpm filter for filename suffix
c <- 0.6 # set cv filter for filename suffix
...
p + geom_histogram(fill="white",color="black",binwidth=0.1) +
  ...
  geom_vline(xintercept = log(3,2)) +
  theme_bw()
ggsave(paste(path,'a7-',n,'rpm-',c,'cv','-rep-rpkm-hist.pdf',sep=""), width=8, height=8)
...
cv06.list.bed <- lapply(cv06.list, function(x) { left_join(x,ttreg[2:5],by="PeakID") } )
# check for NAs
sapply(cv06.list.bed, function(x) all(is.na(x)))
# check
lapply(cv06.list.bed, head)
# re-arrange columns of each df
cv06.list.bed <- lapply(cv06.list.bed, function(x) { cbind(x[1:2],x[11:13],x[3:10])})
names(cv06.list.bed) <- psl.names  # name list elements (recycle previous names)
lapply(cv06.list.bed, head)

## --- export cv-filtered list and tables for each sample ----------------------------------------------------------

# save list of cv-filtered dfs
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A7-all-ttreg/1rpm-cv06/RDa/"
n <- 1 # set rpm filter for filename suffix
c <- 0.6 # set cv filter for filename suffix
save(cv06.list.bed, file = paste(path,'a7-ttreg-',n,'rpm-',c,'cv','-list.RDa',sep=""))
# load
load(paste0(path,'a7-ttreg-1rpm-0.6cv-list.RDa'))

# export each df as .csv file
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A7-all-ttreg/1rpm-cv06/csv/"
suffix <- paste('-',n,'rpm-',c,'cv','-filtered','.csv',sep="")
lapply(names(cv06.list.bed), 
       function(x) write.csv(cv06.list.bed[[x]], file=paste(path,x,suffix,sep=""), row.names=F))
...
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A7-all-ttreg/1rpm-cv06/csv/"
hcp <- read.csv(file = paste(path,'a7-ttreg-1rpm-0.6cv-filtered-merged.csv',sep=""), header =T)
...
##---- plot -------------------------------------------------------------------------------------------------
# set filenames
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A7-all-ttreg/1rpm-cv06/plots/" # set plots path
n <- 1
c <- 0.6
...
# export
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A7-all-ttreg/1rpm-cv06/csv/" # set data path
n <- 1
c <- 0.6
...
# RDa
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A7-all-ttreg/1rpm-cv06/RDa/"  # set data path
...
# load counts 
path <- "/Volumes/JD_WD2/JRD_CBDMLab/1_JRD_CBDM_PROJECTS/ProjectFolders/T-TregEpigenome/Data/1_Experimental/ATAC/Analyses/A7-all-ttreg/1rpm-cv06/RDa/" 
filename <- 'a7-ttreg-1rpm-0.6cv-filtered-rep-avg-counts-log2-rpkm-qn.RDa'
...
c <- 0.6 # set cv cutoff for filenames
...
ttrm <- read.csv(file=paste0(path, 'a7-ttreg-1rpm-0.6cv-filtered-rep-avg-counts-log2-rpkm-qn-peakID-mat.csv'),
...
p <- pheatmap(ttrm, color= niceCols, clustering_distance_rows = 'minkowski', clustering_distance_cols = 'correlation', kmeans_k = 20)
